...
if __name__ == "__main__":
    ...
    for sample_1, sample_2 in zip(samples_1, samples_2):
        ...
        for events_1, report in uproot.iterate(sample_1[args.era], library="ak", step_size=1000, num_workers=4, report=True):
            events_1 = ak.Array([events_1])
            EventScore = EventCate[report.start:report.stop]
            ...
            events_1["DNN_class"] = ak.argmax(EventScore, axis=1, mask_identity=False)
            events_1["DNN0"] = EventScore[:, 0]
            events_1["DNN1"] = EventScore[:, 1]
            events_1["DNN2"] = EventScore[:, 2] 
            events_1["DNN3"] = EventScore[:, 3]
            events_1["DNN4"] = EventScore[:, 4]
            ...
            with uproot.recreate(f"/home/cosine/HHbbgg/MLForge_FollowUp/samples/sample_from_Nitishi_add/{args.era}_{sample_1['name']}/{report.start}.root") as new_file:
