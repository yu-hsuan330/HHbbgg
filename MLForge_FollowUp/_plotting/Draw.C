...
void drawDataMC(std::map<std::string, TH1F> hist, TString var, string unit, float min, float max, string dir){
	...
	if(maxY < signalScale*hist["mc_sig"].GetBinContent(hist["mc_sig"].GetMaximumBin())) maxY = signalScale*hist["mc_sig"].GetBinContent(hist["mc_sig"].GetMaximumBin());
	...
	TLegend *le1 = new TLegend(0.4,0.7,0.92,0.87);
	le1->SetNColumns(2);
	le1->SetFillStyle(0);
	le1->SetBorderSize(0);
	...
	le1->AddEntry(&hist["mc_sig"],Form("signal#times%.f", signalScale),"L");
	// le1->AddEntry(&hist["mc_sig"], "signal", "L");
	...
	if(hrdata->GetMaximum()>2) hrdata->SetMaximum(3);
	
	hrdata->Draw("PE");
	TLine *line1 = new TLine(min,1,max,1);
	//! TLine *line2 = new TLine(min,0.5,max,0.5);
	TLine *line2 = new TLine(min,2,max,2);
	//! TLine *line3 = new TLine(min,1.5,max,1.5);
	TLine *line3 = new TLine(min,3,max,3);
...
void make_DataMC(std::map<std::string, TH1F> &histmap, string json_file="./SampleList.json", string era="22postEE", string var="pt", int binning=50, vector<float> range={0,2,1}, vector<string> unit={"x","y"}){
    ifstream json_sample(json_file.c_str());
    auto infile = nlohmann::json::parse(json_sample);
	...
	TH1F hdata("hdata","", binning, range[0], range[1]);
    if(var.find("diphoton_mass") != std::string::npos) data.Draw(Form("%s>>hdata", var.c_str()), Form("%s*(diphoton_mass<115||diphoton_mass>135)", Data_weight.c_str()));
	else if(var.find("bjet_pair_mass") != std::string::npos) data.Draw(Form("%s>>hdata", var.c_str()), Form("%s*(bjet_pair_mass<105||bjet_pair_mass>145)", Data_weight.c_str()));
    ...
    const int sample_size = infile["MC"].size();
   	...
   	TH1F hmc_sig("hmc_sig","", binning, range[0], range[1]);
	TH1F hmc("hmc","", binning, range[0], range[1]);
	TH1F hHGG("hHGG","", binning, range[0], range[1]);
	TH1F hQCDHT("hQCDHT","", binning, range[0], range[1]);
    ...
    for (nlohmann::json::iterator it = MC.begin(); it != MC.end(); ++it) {
        ...
        // fill hist
		TH1F hist(sample.c_str(), "", binning, range[0], range[1]);
		//TODO optimize!!
		string weight_string =  MC_weight;//Form("%s*%f*%f", MC_weight.c_str(), lumi, xs)
		ch.Draw(Form("%s>>%s", var.c_str(), sample.c_str()), weight_string.c_str());
        histmap[sample] = hist;

        // fill overall hist
        if(sample == "GluGluToHH" || sample == "VBFToHH"){
            ch.Draw(Form("%s>>+hmc_sig", var.c_str()), weight_string.c_str());
        }
        if(sample.find("M125") != std::string::npos){
            ch.Draw(Form("%s>>+hHGG", var.c_str()), weight_string.c_str());
        }
		if(sample.find("QCD_HT") != std::string::npos){
			ch.Draw(Form("%s>>+hQCDHT", var.c_str()), weight_string.c_str());
		}
		ch.Draw(Form("%s>>+hmc", var.c_str()), weight_string.c_str());
...
void DataMC(string json_file="./SampleList.json", string json_file_var="./var.json", string dir="test"){
    
	ifstream infile_var(json_file_var.c_str());
    auto json_var = nlohmann::json::parse(infile_var);

	auto var_list = json_var["var_list"];
	string era = json_var["era"];


	// variable loop
	for(nlohmann::json::iterator it_var = var_list.begin(); it_var != var_list.end(); ++it_var){
		std::map<std::string, TH1F> histmap;
		string var = it_var.key();
		int binning = it_var.value()["nbin"];
		vector<float> range = it_var.value()["range"];
		vector<string> unit = it_var.value()["unit"]; 

		make_DataMC(histmap, json_file, era, var.c_str(), binning, range, unit);
		drawDataMC(histmap, var, unit[0], range[0], range[1], "22_TPS");
...
void Draw(){
	...
	DataMC();
	// make_DataMC(histmap);
	// drawDataMC(std::map<std::string, TH1F> hist, TString var, string unit, float min, float max, string dir);
}
