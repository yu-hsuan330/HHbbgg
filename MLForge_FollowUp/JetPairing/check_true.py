...
if __name__ == "__main__":
    ...
    for sample in samples:
        ...
        #* Define LHE parton
        lhe_lead = ak.zip({"pt": events["lhe_vbf_parton1_pt"], "eta": events["lhe_vbf_parton1_eta"], "phi": events["lhe_vbf_parton1_phi"], "mass": events["lhe_vbf_parton1_mass"], "charge": -999}
                        , with_name="PtEtaPhiMCandidate", behavior=candidate.behavior) 
        lhe_sublead = ak.zip({"pt": events["lhe_vbf_parton2_pt"], "eta": events["lhe_vbf_parton2_eta"], "phi": events["lhe_vbf_parton2_phi"], "mass": events["lhe_vbf_parton2_mass"], "charge": -999}
                        , with_name="PtEtaPhiMCandidate", behavior=candidate.behavior)
        lhe_pair = lhe_lead + lhe_sublead
        ...
        branches = ak.zip({
            ...
            "lhe_lead_pt"       : lhe_lead.pt,
            "lhe_lead_eta"      : lhe_lead.eta,
            "lhe_lead_phi"      : lhe_lead.phi,
            "lhe_lead_mass"     : lhe_lead.mass,
            "lhe_sublead_pt"    : lhe_sublead.pt,
            "lhe_sublead_eta"   : lhe_sublead.eta,
            "lhe_sublead_phi"   : lhe_sublead.phi,
            "lhe_sublead_mass"  : lhe_sublead.mass,
            "lhe_pair_pt"       : lhe_pair.pt,
            "lhe_pair_eta"      : lhe_pair.eta,
            "lhe_pair_phi"      : lhe_pair.phi,
            "lhe_pair_mass"     : lhe_pair.mass
            }
        )
        ...
        if sample["name"] == "VBFToHH":
            ...
            check_results_ = ak.drop_none(check_results)
            check_results_output = ak.zip({
                "event"             : check_results_["bb"].event,
                "lumi_xs"           : check_results_["bb"].lumi_xs,
                "weight"            : check_results_["bb"].weight,
                "n_jets"            : check_results_["bb"].n_jets,    
                
                "diphoton_pt"       : check_results_["bb"].diphoton_pt,
                "diphoton_eta"      : check_results_["bb"].diphoton_eta,
                "diphoton_phi"      : check_results_["bb"].diphoton_phi,
                "diphoton_mass"     : check_results_["bb"].diphoton_mass,
                "diphoton_ptOverM"  : check_results_["bb"].diphoton_pt / check_results_["bb"].diphoton_mass,
                
                "pho_lead_pt"       : check_results_["bb"].pho_lead_pt, 
                "pho_sublead_pt"    : check_results_["bb"].pho_sublead_pt, 
                "pho_lead_eta"      : check_results_["bb"].pho_lead_eta, 
                "pho_sublead_eta"   : check_results_["bb"].pho_sublead_eta, 
                "pho_lead_phi"      : check_results_["bb"].pho_lead_phi, 
                "pho_sublead_phi"   : check_results_["bb"].pho_sublead_phi, 
                "pho_lead_ptOverM"      : check_results_["bb"].pho_lead_pt / check_results_["bb"].diphoton_mass,
                "pho_sublead_ptOverM"   : check_results_["bb"].pho_sublead_pt / check_results_["bb"].diphoton_mass,

                "lhe_lead_pt"       : check_results_["bb"].lhe_lead_pt,
                "lhe_lead_eta"      : check_results_["bb"].lhe_lead_eta,
                "lhe_lead_phi"      : check_results_["bb"].lhe_lead_phi,
                "lhe_lead_mass"     : check_results_["bb"].lhe_lead_mass,
                "lhe_sublead_pt"    : check_results_["bb"].lhe_sublead_pt,
                "lhe_sublead_eta"   : check_results_["bb"].lhe_sublead_eta,
                "lhe_sublead_phi"   : check_results_["bb"].lhe_sublead_phi,
                "lhe_sublead_mass"  : check_results_["bb"].lhe_sublead_mass,
                "lhe_pair_pt"       : check_results_["bb"].lhe_pair_pt,
                "lhe_pair_eta"      : check_results_["bb"].lhe_pair_eta,
                "lhe_pair_phi"      : check_results_["bb"].lhe_pair_phi,
                "lhe_pair_mass"     : check_results_["bb"].lhe_pair_mass
            })
            ...
            for field in store_variables:
                check_results_output["bjet_"+field] = check_results_.bb[field]
                
            for field in store_variables:
                # value = ak.fill_none(getattr(check_results_.jj, field), -999)
                check_results_output["vbfjet_"+field] = check_results_.jj[field]
            
            with uproot.recreate(f"../samples/{args.era}_{sample['name']}_true_cut.root") as new_file:
                if sample["name"] == "data":
                    new_file[TreeName_data] = check_results_output
                else:
                   new_file[TreeName_mc] = check_results_output
