...
if __name__ == "__main__":
    ...
    for sample in samples:
        ...
        if sample["name"] == "VBFToHH":
            ...
            print("# of VBF-events in minitree: {}, {}".format(ak.num(events[(events["VBF_dijet_pt"] > 0)], axis=0), ak.sum(events["weight"]*lumi[args.era]*sample["xs"]*(events["VBF_dijet_pt"] > 0), axis=0)))
            
            print("=====  Cut-based  =====")
            criteria_bb = (events["VBF_dijet_pt"] > 0) & (abs(events["nonRes_lead_bjet_genFlav"]) == 5) & (abs(events["nonRes_sublead_bjet_genFlav"]) == 5)
            criteria_jj = (events["VBF_dijet_pt"] > 0) & (events["VBF_lead_jet_lheMatched"] == 1) & (events["VBF_sublead_jet_lheMatched"] == 1)
