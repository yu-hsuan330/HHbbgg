...
void add_VBFVar(string inpath, string infile, string outpath, string pair1, string pair2, float lumi_xs){
    ...
    auto df_ = df1
                ...
                .Filter("(CMS_hgg_mass > 100) && (CMS_hgg_mass < 180) && (nonRes_dijet_mass > 0) && lead_mvaID > -0.7 && sublead_mvaID > -0.7", "basic selection")
                ...
                .Define("pair_ptOverM",             "pair.Pt()/pair.M()")
...
void edit_minitree_forPairing(){
    std::map<std::string, float> files = {
        ...
        {"22preEE_VBFHHto2B2G_CV_1_C2V_1_C3_1",         7980.4  *   0.00192},
        {"22postEE_VBFHHto2B2G_CV_1_C2V_1_C3_1",        26671.7 *   0.00192},
    ...
    vector<string> jet = {
        "jet1", "jet2", "jet3", "jet4", "jet5", "jet6"//, "jet7", "jet8", "jet9", "jet10"
    };
    ...
    for(int idx1=0; idx1<jet.size(); idx1++){
        for(int idx2=idx1+1; idx2<jet.size(); idx2++){
            for(const auto& n: files){    
                ...
                add_VBFVar("../samples", n.first, "../samples/pairing_6", jet[idx1], jet[idx2], n.second);
            }
        }
    }
