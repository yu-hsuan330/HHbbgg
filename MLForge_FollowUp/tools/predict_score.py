...
samples = [
            ...
            # "22preEE_GJet_Pt20_MGG40to80", "22postEE_GJet_Pt20_MGG40to80",
...
train_model = "MultiClass_DNN_Pairing_v3" #"MultiClass_DNN_VBF_TPS"
version = "ver0121"
...
if __name__ == "__main__":
    ...
    for i, j in zip(samples, lumi_xs):
        print("Start processing file: ", i)
        #* Load the ROOT files
        events = uproot.concatenate(["../../minitree/{}/{}.root:{}".format(version, i, TreeName)], library="ak")
        events = events#[1:5]
        
        # NO LHE info in GGJets sample, so ...
        if "jet1_lheMatched" not in events.fields:
            for k in range(1, 7):
                events["jet{}_lheMatched".format(k)] = -999
            events["VBF_lead_jet_lheMatched"] = -999
            events["VBF_sublead_jet_lheMatched"] = -999
        if "nonRes_lead_bjet_genFlav" not in events.fields:
            for k in range(1, 7):
                events["jet{}_genFlav".format(k)] = -999
            events["nonRes_lead_bjet_genFlav"] = -999
            events["nonRes_sublead_bjet_genFlav"] = -999
        ...
        #* Check purity of the sample
        print("total # of true events (bbjj) in minitree: ", ak.num(events[(events["true_bbjj"]==1)],axis=0), "/", ak.sum(events["weight"]*j*(events["true_bbjj"]==1), axis=0))
        print("total # of true events (bbjj) in minitree: ", ak.num(events[(~ak.is_none(check_results.jj.event))],axis=0), "/", ak.sum(events["weight"]*j*(~ak.is_none(check_results.jj.event)), axis=0))
        criteria = (events["true_bbjj"]==1) & (~ak.is_none(check_results.jj.event))
        print("total # of true events (bbjj) in minitree: ", ak.num(events[criteria],axis=0), "/", ak.sum(events["weight"]*j*(criteria), axis=0))
        ...
        flat_features = ak.flatten(branches[DNN_features])
        ...
        # pair_jj = ak.mask(pair_jj, ~ak.is_none(pair_bb))
        ...
        print("../update_minitree/{}/{}.root".format(version, i))
        #* Store the selected pair to ROOT file
        with uproot.recreate("../update_minitree/{}_/{}.root".format(version, i)) as new_file:
            new_file[TreeName] = output




        # print(candidate_bb.lead_btagPNetB+candidate_bb.sublead_btagPNetB)
        # print(ak.firsts(candidate_bb.lead_btagPNetB+candidate_bb.sublead_btagPNetB))
        # print(candidate_bb.lead_btagPNetB+candidate_bb.sublead_btagPNetB)




        # pair_bbjj = ak.cartesian({"bjet": candidate_bb, "vbfjet": candidate_jj})
        
        # print(pair_bbjj["bjet"].lead_pt, pair_bbjj["vbfjet"].lead_pt)
        # results = ak.firsts(pair_bbjj[deltaRCut & idxCut])
        # print(results["bjet"].lead_pt, results["vbfjet"].lead_pt)

        # print(results)
        # results = ak.drop_none(results)
        
             
        # print(pair_bbjj["bjet"].pair_mass, pair_bbjj["vbfjet"].pair_mass)
        ...
        # print("[DNN] #/yield of bb pair(): ", ak.num(pair_bb, axis=0), "/", j*ak.sum(pair_bb["weight"], axis=0))
        ...
        # print("[DNN] #/yield of bbjj pair(VBF yield): ", ak.num(results, axis=0), "/", j*ak.sum(results["bjet"]["weight"], axis=0))
        # print("[DNN] #/yield of select matched bb in matched bbjj", ak.sum(results["bjet"].true_bbjj*results["bjet"].true_bjet_pair), "/", j*ak.sum(results["bjet"].true_bbjj*results["bjet"].true_bjet_pair*results["bjet"]["weight"]))
        # print("[DNN] #/yield of select matched jj in matched bbjj", ak.sum(results["vbfjet"].true_bbjj*results["vbfjet"].true_vbfjet_pair), "/", j*ak.sum(results["vbfjet"].true_bbjj*results["vbfjet"].true_vbfjet_pair*results["vbfjet"]["weight"]))
