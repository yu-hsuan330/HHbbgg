...
OutputDirName = "results/Pairing_vbf_v3_10jets" # All plots, models, config file will be stored here
...
Branches = [
    ...
    "pair_pt", "pair_eta", "pair_phi", "pair_mass", "pair_ptOverM", "pair_DeltaR", "pair_DeltaPhi", "pair_eta_prod", "pair_eta_diff", "pair_Cgg",
    ...
    "nonRes_dijet_mass", "nonRes_dijet_eta", "nonRes_DeltaR_jg_min", #"fixedGridRhoAll"
...
Processes = [
    {
    "Class": "b-jet pair",
    "path": ["/home/cosine/HHbbgg/MLForge_FollowUp/JetPairing/samples/pairing_6/22*3.root"],
...
MVAs = [
    {
        ...
        "features":[
            ...
            "pair_Cgg", "pair1_phi", "pair2_phi", #"fixedGridRhoAll"
        ],
        "features_unit":[
            ...
            "$N_{jets}$", "$p_{T}^{jj}/M_{jj}$", "$\eta_{jj}$", # "$M_{jj} (GeV)$",
            "$\Delta R(j_1, j_2)$", "$\Delta \Phi(j_1, j_2)$", "$\eta_{j_1} X \eta_{j_2}$", "$\Delta\eta(j_1, j_2)$", 
            "$C_{\gamma\gamma}$", "lead $\Phi_{j}$", "sublead $\Phi_{j}$", #"rho"
        ],
        "feature_bins":[ 
            ...
            np.linspace(2, 10, 9), np.linspace(0, 5, 31), np.linspace(-5, 5, 31), #np.linspace(0, 3000, 31),
            31, 31, np.linspace(-20, 15, 21), 31,
            31, 15, 15, #np.linspace(0, 60, 31)
        ],
        "hyperopt": False,
        "Algorithm": "DNN",
        "Scaler":"MinMaxScaler", #Scaling for features before passing to the model training
        
        "DNNDict":{ "ModelParams":{"epochs": 200, "batchsize": 1024, "input_dim": (18,), "output_dim": len(Classes)},
                    'compile': {'loss':'categorical_crossentropy', 'metrics':['accuracy']}, #,'optimizer':Adam(learning_rate=0.004)
