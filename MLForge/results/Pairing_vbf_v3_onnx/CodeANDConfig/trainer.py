    ...
    for MVA in Conf.MVAs:
        ...
        if MVA["Algorithm"] == "DNN":
            ...
            def create_DNN_model(params):
                model = Sequential()
                model.add(Input(shape=params["input_dim"], name="input"))
                # model.add(Masking(mask_value=-999))  # Masking layer to ignore -1 values
                model.add(normalizer)
                model.add(Dense(params["num_units_1"], kernel_initializer=params["kernel_initializer"], activation='relu'))
                model.add(Dropout(params["dropout_rate"]))
                model.add(Dense(params['num_units_2'], kernel_initializer=params["kernel_initializer"], activation="relu"))
                model.add(Dropout(params['dropout_rate']))
                model.add(Dense(params['num_units_3'], kernel_initializer=params["kernel_initializer"], activation="relu"))
                model.add(Dropout(params['dropout_rate']))
                model.add(Dense(params['num_units_4'], kernel_initializer=params["kernel_initializer"], activation="relu"))
                model.add(Dense(params["output_dim"], activation="softmax", name="output"))
                
                optimizer = Adam(learning_rate=params['learning_rate'])
                model.compile(loss=MVA["DNNDict"]["compile"]["loss"], optimizer=optimizer, metrics=MVA["DNNDict"]["compile"]["metrics"])
            ...
            if MVA["hyperopt"]:      
                ...
                modelDNN = create_DNN_model({**MVA["DNNDict"]["ModelParams"], **MVA["DNNDict"]["model"]})
            ...
            modelDNN.export(Conf.OutputDirName+"/"+MVA["MVAtype"]+"/"+MVA["MVAtype"]+"_modelDNN")
        ...
        plot_Features_Scaled(MVA, normalizer(x_train).numpy(), y_train, w_train, Conf)
