...
def plot_Features2(MVA, x_train, y_train, weight, Conf):

    # print(np.isinf(x_train).sum(), np.isnan(x_train).sum(), (x_train == 0).sum())
    ...
    for i, feature in enumerate(MVA["features"]):
        ...
        for j, Class in enumerate(Conf.Classes):
            ...
            ax.hist(feature_Class, label=Class, bins=30, density=True,
                    histtype='step', alpha=0.7, linewidth=4, color=Conf.ClassColors[j])
...
def plot_VarImportance(MVA, model, Conf, X_train, X_test):
    ...
    if MVA["Algorithm"] == "XGB":
        ...
        explainer = shap.SamplingExplainer(model.predict, X_train[:553600])
        # background_sample = shap.sample(X_train, 100)
        # explainer = shap.KernelExplainer(model.predict, background_sample)
        shap_values = explainer.shap_values(X_test[:553600], nsamples=10)
        
        # Plot the feature importance
        shap.summary_plot(shap_values, X_test, feature_names=MVA["features_unit"], show=False, color="blue")
...
def plot_correlation_matrix(MVA, X_train, Conf):
    ...
    if isinstance(X_train, np.ndarray):
        X_train = pd.DataFrame(X_train, columns=MVA["features_unit"])
    corr = X_train.corr()
    ...
    sns.heatmap(corr, cmap='coolwarm', ax=ax, xticklabels=MVA["features_unit"], yticklabels=MVA["features_unit"])
    # pltSty(ax, xName = "Predicted Class", yName = "Actual Class", yAuto = False, MajTickLength = 0, MinTickLength = 0)

    # Set titles and labels
    # ax.set_title(title)
    fig.savefig(Conf.OutputDirName+"/"+MVA["MVAtype"]+"/"+"CorrMat_"+MVA["MVAtype"]+".pdf", bbox_inches='tight')
    fig.savefig(Conf.OutputDirName+"/Plots/"+"CorrMat_"+MVA["MVAtype"]+".pdf", bbox_inches='tight')
