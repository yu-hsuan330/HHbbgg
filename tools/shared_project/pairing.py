...
def apply_VBFHH_pairing(events, model, sc):
    ...
    for idx in range(1, 7):
        ...
        # events[f"jet{idx}_btagPNetQvG"] = -999
    
    #* Define jet collection
    jet1 = ak.zip({"pt": events["jet1_pt"], "eta": events["jet1_eta"], "phi": events["jet1_phi"], "mass": events["jet1_mass"], "charge": events["jet1_charge"], "btagPNetB": events["jet1_btagPNetB"], "btagPNetQvG": events["jet1_btagPNetQvG"], "genFlav": events["jet1_genFlav"]})
    jet2 = ak.zip({"pt": events["jet2_pt"], "eta": events["jet2_eta"], "phi": events["jet2_phi"], "mass": events["jet2_mass"], "charge": events["jet2_charge"], "btagPNetB": events["jet2_btagPNetB"], "btagPNetQvG": events["jet2_btagPNetQvG"], "genFlav": events["jet2_genFlav"]})
    jet3 = ak.zip({"pt": events["jet3_pt"], "eta": events["jet3_eta"], "phi": events["jet3_phi"], "mass": events["jet3_mass"], "charge": events["jet3_charge"], "btagPNetB": events["jet3_btagPNetB"], "btagPNetQvG": events["jet3_btagPNetQvG"], "genFlav": events["jet3_genFlav"]})
    jet4 = ak.zip({"pt": events["jet4_pt"], "eta": events["jet4_eta"], "phi": events["jet4_phi"], "mass": events["jet4_mass"], "charge": events["jet4_charge"], "btagPNetB": events["jet4_btagPNetB"], "btagPNetQvG": events["jet4_btagPNetQvG"], "genFlav": events["jet4_genFlav"]})
    jet5 = ak.zip({"pt": events["jet5_pt"], "eta": events["jet5_eta"], "phi": events["jet5_phi"], "mass": events["jet5_mass"], "charge": events["jet5_charge"], "btagPNetB": events["jet5_btagPNetB"], "btagPNetQvG": events["jet5_btagPNetQvG"], "genFlav": events["jet5_genFlav"]})
    jet6 = ak.zip({"pt": events["jet6_pt"], "eta": events["jet6_eta"], "phi": events["jet6_phi"], "mass": events["jet6_mass"], "charge": events["jet6_charge"], "btagPNetB": events["jet6_btagPNetB"], "btagPNetQvG": events["jet6_btagPNetQvG"], "genFlav": events["jet6_genFlav"]})
    ...
    deltaRCut = ((vector.Spatial.deltaR(pair_bb.lead, pair_bb.lead) > 0.4) &
                (vector.Spatial.deltaR(pair_bb.lead, pair_bb.sublead) > 0.4) &
                (vector.Spatial.deltaR(pair_bb.sublead, pair_bb.lead) > 0.4) &
                (vector.Spatial.deltaR(pair_bb.sublead, pair_bb.sublead) > 0.4))
    
    # for idx1, idx2 in [(pair_bb.lead, candidate_jj.lead),(pair_bb.lead, candidate_jj.sublead),
    #                    (pair_bb.sublead, candidate_jj.lead),(pair_bb.sublead, candidate_jj.sublead)]:
    #     deltaRCut = vector.Spatial.deltaR(idx1, idx2) > 0.4
...
if __name__ == "__main__":
    ...
    new = apply_VBFHH_pairing(data_list, model, sc)
    fig, ax = plt.subplots(1, 1, figsize=(7.5, 5))
    new = new[new["nonRes_dijet_mass_vbfpairing"] > 0]
    ax.hist(new["nonRes_dijet_mass_vbfpairing"], bins=50, density=False, histtype='step', alpha=0.7, linewidth=4)
    pltSty(ax, xName="nonRes_dijet_mass_vbfpairing", yName="Events", yAuto=False)
    fig.savefig("nonRes_dijet_mass_vbfpairing.pdf")
    # print(new["nonRes_dijet_pt"])
    # print(new["nonRes_dijet_pt_"])
    # print(data_list["pt"])