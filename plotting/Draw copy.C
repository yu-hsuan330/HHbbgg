...
void drawDataMC(std::map<std::string, TH1F> hist, TString var, string unit, float min, float max, string dir){
	...
	SetHistColor(&hist["VHtoGG_M125"], "#735CDD");
	SetHistColor(&hist["GJet_Pt20to40"], "#37A3D2");
	SetHistColor(&hist["DD_QCDGJets"], "#37A3D2");
	SetHistColor(&hist["VBFHtoGG_M125"], "#F3C568");
	SetHistColor(&hist["GluGluHtoGG_M125"], "#70877F");
...
void Draw(){
	...
	std::map<std::string, TH1F> histmap;
	make_DataMC(histmap, json_file, "22postEE", "nonRes_dijet_mass", 31, 70, 190);
	drawDataMC(histmap, "nonRes_dijet_mass", "nonRes_dijet_mass", 70, 190, "plzzz_");

	make_DataMC(histmap, json_file, "22postEE", "CMS_hgg_mass", 41, 100, 180);
	drawDataMC(histmap, "CMS_hgg_mass", "CMS_hgg_mass", 100, 180, "plzzz_");

	make_DataMC(histmap, json_file, "22postEE", "nonRes_dijet_pt", 51, 0, 1000);
	drawDataMC(histmap, "nonRes_dijet_pt", "dijet p_{T} [GeV]", 0, 1000, "plzzz_");

	make_DataMC(histmap, json_file, "22postEE", "pt", 51, 0, 1000);
	drawDataMC(histmap, "pt", "diphoton p_{T} [GeV]", 0, 1000, "plzzz_");
}