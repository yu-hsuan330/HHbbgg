...
void drawDataMC(std::map<std::string, TH1F*> hist, TString var, string unit, float min, float max, string dir){
	...
	hs->Add(hist["GGJets"]);	
	hs->Add(hist["GjetPt40"]);
	hs->Add(hist["GjetPt20to40"]);
    hs->Add(hist["QCD"]);
	hs->Add(hist["HGG"]);
	...
	hist["GGJets"]->SetFillColor(TColor::GetColor("#0B66C1"));
	hist["GGJets"]->SetLineColor(TColor::GetColor("#0B66C1"));
	hist["HGG"]->SetFillColor(TColor::GetColor("#BCC2AD"));
	hist["HGG"]->SetLineColor(TColor::GetColor("#BCC2AD")); 
	hist["GjetPt40"]->SetFillColor(TColor::GetColor("#37A3D2"));
	hist["GjetPt40"]->SetLineColor(TColor::GetColor("#37A3D2"));
	hist["GjetPt20to40"]->SetFillColor(TColor::GetColor("#EC8C6F"));
	hist["GjetPt20to40"]->SetLineColor(TColor::GetColor("#EC8C6F"));
	hist["QCD"]->SetFillColor(TColor::GetColor("#F3C568"));
	hist["QCD"]->SetLineColor(TColor::GetColor("#F3C568"));
	hs->Draw("hist");
	...
	le1->AddEntry(hist["GjetPt20to40"],"#gamma+jets (40 > P_{T} > 20)","f");
	le1->AddEntry(hist["GjetPt40"],"#gamma+jets (P_{T} > 40)","f");
	le1->AddEntry(hist["GGJets"],"#gamma#gamma+jets","f");
    le1->AddEntry(hist["QCD"],"QCD","f");
...
void make_hist(std::map<std::string, TH1F*> &histmap, string json_file="./SampleList.json", string era="22postEE", string var="pt", int binning=50, float min=0, float max=400){
    ...
    data.Draw(Form("%s>>hdata", var.c_str()));
    //* MC
    ...
    for (nlohmann::json::iterator it = MC.begin(); it != MC.end(); ++it) {
        ...
        // fill hist
        hist[i] = new TH1F(sample.c_str(), "", binning, min, max);
		
		ch[i]->Draw(Form("%s>>%s", var.c_str(), sample.c_str()), Form("weight*%f*%f", lumi, xs));
        histmap[sample] = hist[i];

        // fill overall hist
        if(sample == "GluGluToHH" || sample == "VBFToHH"){
            ch[i]->Draw(Form("%s>>+hmc_sig", var.c_str()), Form("weight*%f*%f", lumi, xs));
        }
        if(sample.find("M125") != std::string::npos){
            ch[i]->Draw(Form("%s>>+hHGG", var.c_str()), Form("weight*%f*%f", lumi, xs));
        }
		ch[i]->Draw(Form("%s>>+hmc", var.c_str()), Form("weight*%f*%f", lumi, xs));
...
void draw_testt(){
    ...
    for(int i=0; i<var.size(); i++){
        ...
        drawDataMC(histmap, var[i][0], var[i][1], range[i][1], range[i][2], "test");
    }
